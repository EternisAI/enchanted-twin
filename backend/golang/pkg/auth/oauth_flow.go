...
func callbackHandler(ctx context.Context, logger *log.Logger, store *db.Store, r *http.Request) (string, interface{}, error) {
	...
	if r.URL.Query().Get("status") == "success" {
		...
		accountID := r.URL.Query().Get("connectedAccountId")
		provider := r.URL.Query().Get("appName")

		username, err := CompleteOAuthFlowComposio(ctx, logger, store, accountID, provider)
		if err != nil {
