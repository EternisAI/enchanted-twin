...
func TestIsSystemMessage(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "chat name with angle brackets",
			text:     "Eternis <> Bala",
			expected: true,
		},
		{
			name:     "phone number with whatsapp domain",
			text:     "16672940017@s.whatsapp.net",
			expected: true,
		},
		{
			name:     "group JID",
			text:     "120363123456789@g.us",
			expected: true,
		},
		{
		...
		},
		{
			...
			text:     "Hey @Bala - made this group so you can interact with my co-founders as well",
			expected: false,
		},
		{
			name:     "message with phone number in context",
			text:     "Call me at +1234567890 when you get this",
			expected: false,
		},
		{
			name:     "message with email",
			text:     "Send it to john@example.com",
			expected: false,
		},
		{
			name:     "empty message",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := isSystemMessage(tt.text)
			if result != tt.expected {
				t.Errorf("isSystemMessage(%q) = %v, expected %v", tt.text, result, tt.expected)
			}
		})
	}
}
