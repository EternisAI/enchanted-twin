...
func (r *mutationResolver) UpdateFeedback(ctx context.Context, messageID string, feedback model.FeedbackType) (bool, error) {
	err := r.TwinChatService.UpdateMessageFeedback(ctx, messageID, feedback)
	if err != nil {
		return false, fmt.Errorf("failed to update message feedback: %w", err)
	}
