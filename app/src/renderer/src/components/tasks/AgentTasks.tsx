        ...
        <Card className="p-6 w-full">
            ...
            <div className="flex flex-col gap-4">
                {agentTasks.length === 0 ? (
                    ...
                    agentTasks.filter(task => !task.endedAt).map((task) => (
                        <AgentTaskRow
                            key={task.id}
                            task={task}
    ...
    return (
        <div className="flex flex-col gap-2 rounded-xl border border-gray-300 p-4 w-full">
            ...
            <div className="flex gap-4 text-xs text-muted-foreground">
                ...
                {task.completedAt && <span>Completed {formatDistanceToNow(new Date(task.completedAt))} ago</span>}
                {task.output && <span>Output: {task.output}</span>}
            </div>
        </div>
    )
}
